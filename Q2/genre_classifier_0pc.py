...
Pred_Loader = Data.DataLoader(Pred_Data, batch_size = batch_size, shuffle = True)
...
with torch.no_grad():
    for i, batch in enumerate(tqdm(Pred_Loader, desc = 'Iteration')):
        ...
        output = nn.functional.softmax(output['logits'], dim = 1).data.cpu().numpy().argmax()
